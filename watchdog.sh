...
[ -n "$DOTOKEN" ] || { echo "DO token env variable must be set" ; exit 1; }
[ -n "$RECORDID" ] || { echo "Record ID env variable must be set" ; exit 1; }
...
doctl compute domain records update $DNSZONE --record-name minecraft --record-data $PUBLICIP --record-type A --record-id $RECORDID --record-ttl 30 -t $DOTOKEN
